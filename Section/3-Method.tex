...
\begin{figure}[t]
    ...
    \label{fig:tasks}
\end{figure}
To investigate how acute stress affects different forms of interaction in VR, we implemented three tasks: \textit{target acquisition}, \textit{reading aloud}, and \textit{Tower of London} in a unified immersive environment (~\autoref{fig:tasks}). All tasks were developed in Unity 2022.3.62f3 and deployed on a Meta Quest Pro headset using optical hand tracking (XR Hands) for controller-free interaction. We recorded physiological data with a Polar H10 chest strap, which continuously recorded heart rate and beat-to-beat (RR) intervals using Bluetooth throughout the study; processing details are given in ~\ref{sec:stress-validation}.
\subsection{Tasks}
...
\begin{figure*}[!t]
    ...
    \caption{Experimental procedure. Abbreviations in the figure: Target Acq. = target acquisition; Speech prep. = speech preparation; Quest. = questionnaire. Round 1 is Post-Stress, and Round 2 is Post-Recovery Measurement.}
    \label{fig:procedure}
\end{figure*}
To induce stress, we adopted a VR adaptation of the Trier Social Stress Test (VR-TSST). The session comprised preparation and training, baseline measurement, VR-TSST stress induction, post-stress measurement, a 10-minute recovery period, and post-recovery measurement, and recovery---and lasted approximately 60--70~minutes per participant. An overview is shown in~\autoref{fig:procedure}.
...
\subsubsection{Resting Period and Baseline Measurement}
...
Following the post-stress measurement round, participants were given a 10~minute recovery period. To improve comfort and reduce headset fatigue, they were asked to remove the headset and rest quietly. After the recovery period, participants completed the second measurement round and filled in the STAI questionnaire. This round is referred to as Post-Recovery.
